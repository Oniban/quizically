...
- [x] Publication retries reuse a per-editor operation ID and recover the original quiz. Concurrent retries cannot create duplicate quizzes. If content changes after an earlier publication succeeded, the editor links to that quiz instead of creating another one.
...
The editor generates the key on its first valid publish attempt and retains it through errors, edits, and same-account inline reauthentication. A new editor session gets a new key. Neither the key nor the draft survives a reload or navigation away; retry on the same page when a response is lost. On a content conflict, use the supplied link to inspect the already published quiz before starting another one.
